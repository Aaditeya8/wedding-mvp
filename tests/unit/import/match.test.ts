...
const EXISTING: ExistingFamily[] = [
  ...
  { id: "f2", name: "Deshmukh", email: "", members: ["Deshmukh masa", "Bunty", "Chintu"] },
  { id: "f3", name: "The Khans", email: "khan@x.com", members: ["Zoya Khan"] },
  { id: "f4", name: "Patil Family", email: "", members: ["Guest 1", "Guest 2"] },
];
...
describe("normalizeFamilyName", () => {
  it("strips family words, articles, honorifics, possessives and plurals", () => {
    ...
    expect(normalizeFamilyName("Deshmukh ji & family")).toBe("deshmukh");
...
describe("matchExisting", () => {
  ...
  it("matches by normalised household name when there is no email", () => {
    expect(matchExisting(row({ name: "Deshmukh Family" }), EXISTING)).toEqual({ id: "f2", reason: "name" });
  ...
  it("never matches by name or people when both sides have different emails", () => {
    ...
    expect(matchExisting(row({ name: "Whoever", email: "new@x.com", members: ["Bunty", "Chintu"] }), [{ ...EXISTING[1], email: "deshmukh@x.com" }])).toBeNull();
  });
  it("matches by member overlap when names differ", () => {
    expect(matchExisting(row({ name: "Masa's group", members: ["Bunty", "Chintu", "New Person"] }), EXISTING)).toEqual({ id: "f2", reason: "members" });
  });
  it("does not match on weak overlap, placeholders, or unrelated rows", () => {
    ...
    expect(matchExisting(row({ name: "Big Group", members: ["Bunty", "A", "B", "C", "D"] }), EXISTING)).toBeNull();
